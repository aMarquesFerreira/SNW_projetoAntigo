...
import IPathService from "./IServices/ISneakersService";
...
import IPathRepo from "./IRepos/ISneakersRepo";
...
export default class ImporterService implements IImporterService {
    ...
    public async importData(filePath: { filePath: string; }, res: Response, next: NextFunction) {
        ...
        parser.parseString(xml_string, function(error, result) {
            ...
            try{
                paths = result['GlDocumentInfo']['world'][0]['GlDocument'][0]['GlDocumentNetwork'][0]['Network'][0]['Paths'][0]['Path'];
            }catch(error){ 
                console.log('There are no Paths in the XML File.')
            }
            try{
        ...
        await this.importPaths(paths, nodes, res, next);
        if (res.statusCode == 402) {
            return;
        }
    ...
    public async importPaths(paths: any, nodes: any, res: Response, next: NextFunction) {
        for (let index = 0; index < paths.length; index++) {
            let pathNodes = paths[index]['PathNodes'][0]['PathNode'];
            let newSegments = []; 
            
            
            for (let index2 = 0; index2 < pathNodes.length; index2++) {
                let initNode = this.findNodeShortName(pathNodes[index2].ATTR.NodeInit, nodes);
                let endNode = this.findNodeShortName(pathNodes[index2].ATTR.NodeEnd, nodes);
                
                if(initNode != null && endNode != null){ 
                    newSegments.push({
                                        initialNode : initNode,
                                        finalNode : endNode,
                                        duration : parseInt(pathNodes[index2].ATTR.Duration, 10),
                                        distance : parseInt(pathNodes[index2].ATTR.Distance, 10)
                                    });
                }
            }           
            
            try {
                let id = paths[index].ATTR.key.split(':')[1];
                const pathOrError = await this.pathService.createPath({
                    pathId : id,
                    isEmpty : (paths[index].ATTR.IsEmpty == "true"),
                    segments : newSegments,
                } as IPathDTO) as Result<IPathDTO>;
                    
                if (pathOrError.isFailure) {
                    return res.status(402).send('There was an error in the Paths')
                }
            } catch (e) {
                return res.status(402).send('There was an error in the Paths')
            }                
    ...
    private findPathIndex(pathKey: string, paths: any): number {
        for (let index1 = 0; index1 < paths.length; index1++) {
            if (paths[index1].ATTR.key === pathKey) {
                return index1;
            }
        }
        return null;
